...
passed to native calls can be printed on the standard output. To enable this, just pass the `-XDjextract.trace.downcalls`
property to the launcher used to start the application. Below we show an excerpt of the output when running the
[OpenGL example](samples/opengl) with tracing support enabled:
